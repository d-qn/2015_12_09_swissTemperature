...
hline.size <- 0.5 
line.text.size <- 4
title <- "2015: année record de chaleur en Suisse"
footer <- "swissinfo.ch | source: MétéoSuisse"
monthly.average.label <- "Moyenne mensuelle 1864-2014"
overall.average.label <- "moyenne 1864-2014: "
...
gridFormat <- function(gg, top = title, bottom = footer) {
    grid.arrange(gg, 
      ...
      bottom = textGrob(bottom, x = 0.98, y = 1, vjust = 1, hjust = 1, 
        gp = gpar(fontsize = 9, fontfamily = fontR, col = "#737373"))
    )
}

plot.intro <- function(df, plot.title, average, y.range) {
  ...
  bg.p4 <- bg.p3 + 
   ...
   invisible(sapply(1:5, function(i) gridFormat(bg.p4, top = title)))
...
plot.year <- function(df, y, bg.p, group.colors) {
   ...
   # add horizontal line for the latest max yearly temp
    ...
    main.p <- p2 + 
      ...
      annotate("text", x = 10.5, y = y.range[2] - y.range[2]/10, label = y,  
        family = fontR, size = 20, color = group.colors[unlist(ddd[1,'colorGroup'])])
    ...
    gridFormat(main.p2)
    if(y == 2015) sapply(1:12, function(i) gridFormat(main.p2))  
}

render.gif <- function(data, stations, plot.title) {
  ...
  saveGIF({
    bg.p <- plot.intro(df, plot.title, average, y.range)
    sapply(unique(df$year), function(y) plot.year(df, y, bg.p, group.colors))
  }, interval = 0.4, movie.name = paste0("swissTemperature_", paste(stations, collapse ="_"), ".gif"), ani.width = 640, ani.height = 720)
}
```

## Create GIF
```{r}
#render.gif(data, stations = "LUG", plot.title = "Températures moyennes mesurées à Lugano (sud de la Suisse)")
# render.gif(data, stations = c("SAE"), 
#   plot.title = "Température moyenne mesurée à Säntis (2502m, nord des Alpes)")

render.gif(data, stations = c("SAE", "ENG"), 
  plot.title = "Température moyenne mesurée en altitude, à Säntis (2502m) et Engelberg (1036m)\n")
```


## Compute the average annual temperature and its deviation from 1961-1990 average
```{r}
yearly.average <- data %>% group_by(year) %>% summarise(yearlyMean = mean(temp, na.rm = T)) %>% ungroup()
average <- data %>% filter(year >= 1961, year <= 1990) %>% 
  summarise(average = mean(temp, na.rm = T)) %>% ungroup() %>% unlist(use.names = F)

yearly.average$ecart <- yearly.average$yearlyMean - average
write.csv(yearly.average, file = "input/ecartTemperatureMoyennes_normes_1961_1990.csv", row.names = F)

average <- data %>% filter(year >= 1981, year <= 2010) %>% 
  summarise(average = mean(temp, na.rm = T)) %>% ungroup() %>% unlist(use.names = F)

yearly.average$ecart <- yearly.average$yearlyMean - average
write.csv(yearly.average, file = "input/ecartTemperatureMoyennes_normes_1981_2010.csv", row.names = F)
```
