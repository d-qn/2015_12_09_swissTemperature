...
library(gridExtra) # for the graphic header & footer
library(scales)
library(ggplot2)
library(extrafont) # just to customize the font
library(animation) # for  gif/video output
...
## URL of Swiss meterological stations
url_stations <- c(
...
tables <- lapply(basename(url_stations), function(filen) {
...
gridFormat <- function(gg, top = "2015: record warm year in Switzerland" ,
  bottom = "swissinfo.ch | source: MétéoSuisse") {
  grid.arrange(gg,
...
render.gif <- function(
  ...
  output.file = paste0("swissTemperature_", paste(stations, collapse ="_"), ".gif"),
...
render.gif(data,
   stations = stations,
   plot.title = "Average temperature at altitude in Säntis (2502m) and Engelberg (1036m)\n",
   monthly.average.label = "Monthly average 1864-2014",
   overall.average.label = "Average 1864-2014",
   fontv,
   output.file = paste0("swissTemperature_", paste(stations, collapse ="_"), "_en.gif"),
   createMovieTeaser = F
)
